...
int find_builtin(info_t *info)
{
	...
	for (i = 0; built_in_table[i].type; i++)
		if (str_cmp(info->argv[0], built_in_table[i].type) == 0)
		{
			info->line_count++;
			built_in_ret = built_in_table[i].func(info);
			break;
		}
	return (built_in_ret);
}
