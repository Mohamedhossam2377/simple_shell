#include "shell.h"

/**
 * add_node_start - add node to the start of list
 *
 * @head: address of pointer to head node
 * @str: string
 * @num: node index
 *
 * Return: size of list
 */
list_t *add_node_start(list_t **head, const char *str, int num)
{
	list_t *new_head;

	if (!head)
		return (NULL);
	new_head = malloc(sizeof(list_t));
	if (!new_head)
		return (NULL);
	mem_set((void *)new_head, 0, sizeof(list_t));
	new_head->num = num;
	if (str)
	{
		new_head->str = str_dup(str);
		if (!new_head->str)
		{
			free(new_head);
			return (NULL);
		}
	}
	new_head->next = *head;
	*head = new_head;
	return (new_head);
}

/**
 * add_node_end - add node to the end of list
 *
 * @head: address of pointer to head node
 * @str: string
 * @num: node index
 *
 * Return: size of list
 */
list_t *add_node_end(list_t **head, const char *str, int num)
{
	list_t *new_node, *node;

	if (!head)
		return (NULL);

	node = *head;
	new_node = malloc(sizeof(list_t));
	if (!new_node)
		return (NULL);
	mem_set((void *)new_node, 0, sizeof(list_t));
	new_node->num = num;
	if (str)
	{
		new_node->str = str_dup(str);
		if (!new_node->str)
		{
			free(new_node);
			return (NULL);
		}
	}
	if (node)
	{
		while (node->next)
			node = node->next;
		node->next = new_node;
	}
	else
		*head = new_node;
	return (new_node);
}

/**
 * print_str_list - print only string in list_t
 *
 * @ptr: pointer to first node
 *
 * Return: size of list
 */
size_t print_str_list(const list_t *ptr)
{
	size_t i = 0;

	while (ptr)
	{
		_puts(ptr->str ? ptr->str : "(nil)");
		_puts("\n");
		ptr = ptr->next;
		i++;
	}
	return (i);
}

/**
 * del_node_index - delete node at given index
 *
 * @head: address of pointer at first node
 * @index: index of node to delete
 *
 * Return: 1 on success or 0 if fail
 */
int del_node_index(list_t **head, unsigned int index)
{
	list_t *node, *pre_node;
	unsigned int i = 0;

	if (!head || !*head)
		return (0);

	if (!index)
	{
		node = *head;
		*head = (*head)->next;
		free(node->str);
		free(node);
		return (1);
	}
	node = *head;
	while (node)
	{
		if (i == index)
		{
			pre_node->next = node->next;
			free(node->str);
			free(node);
			return (1);
		}
		i++;
		pre_node = node;
		node = node->next;
	}
	return (0);
}

/**
 * free_list - delete node at given index
 *
 * @ptr_head: address of pointer at head node
 *
 * Return: nothing
 */
void free_list(list_t **ptr_head)
{
	list_t *node, *next_node, *head;

	if (!ptr_head || !*ptr_head)
		return;
	head = *ptr_head;
	node = head;
	while (node)
	{
		next_node = node->next;
		free(node->str);
		free(node);
		node = next_node;
	}
	*ptr_head = NULL;
}
